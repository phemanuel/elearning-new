...
class MailController extends Controller
{
    ...
    public function emailVerifyInstructor()
    {
        $email_address = session('email');
        $email_message = session('email_message');  
        return view('students.auth.email-verify-instructor')
                ->with('email', $email_address)
                ->with('message', $email_message);
    }
    ...
    public function emailNotVerifyInstructor(Request $request)
    {
        $validatedData = $request->validate([            
            'email' => 'required|email',            
        ]);
        ...
        return redirect()->route('resend-verification-email-instructor');     
    }
    ...
    public function resendEmailVerificationInstructor()
    {
        $email = session('email');
    ...
    public function resendVerificationInstructor(Request $request)
    {
        $email = $request->input('email');
        session(['email' => $email]);
    ...
    public function emailVerifyDoneInstructor($token)
    {
        try {
            // Find the user with the given token
